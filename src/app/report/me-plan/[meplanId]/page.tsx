...
const MEPlan = ({ params }: { params: { meplanId: string } }) => {
    ...
    const rowCretor = (item: any, from?: string) => {
        return <div key={item.id} className="flex border-l border-gray-500">
            ...
            <p className="flex-grow w-32 p-1 border-r border-b border-gray-500">{item.code}</p>
            <p className="flex-grow w-32 p-1  border-r border-b border-gray-500">{item.defination}</p>
                            ...
                            <div className='flex flex-col bg-gray-50 '>
                                {
                                    data?.goals?.length > 0 && data.goals.map((goal: any, i: any) =>
                                        <div key={goal.id}>
                                            <p className='w-full border-b border-l border-r border-gray-500 p-1 '>Goal</p>
                                            {goal?.indicators.length > 0 && goal?.indicators.map((indicator: any, index: any) => { return rowCretor(indicator) })}
                                            {goal?.outcomes.length > 0 && goal?.outcomes.map((outcome: any, index: any) => {
                                                return <>
                                                    <p className='w-full border-b border-l border-r border-gray-500 p-1 '>Outcome</p>
                                                    {rowCretor(outcome)}</>
                                            })}

                                            {goal?.outputs.length > 0 && goal?.outputs.map((output: any, index: any) => {
                                                return <>
                                                    <p className='w-full border-b border-l border-r border-gray-500 p-1 '>Output</p>
                                                    {rowCretor(output)}</>
                                            })}
                                        </div>
                                    )}
